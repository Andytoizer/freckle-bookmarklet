...
const Builder: React.FC<BuilderProps> = ({ initial }) => {
  ...
  const playPrompt = (p: { prompt: string; where: string }) => `${p.prompt}\n\n${p.where} in the Freckle workbook "Freckle bookmark", the workflow whose webhook URL is ${webhook || '<your webhook URL>'}.\n\n${PLAY_RULES}\n\nBefore you tell me it's done, send one real URL of that type through the webhook, run it end to end, and confirm the run completes and the row has the new fields filled in. If anything fails, the setup is wrong: fix it and rerun until it completes cleanly. Remove any test rows you created.`;
